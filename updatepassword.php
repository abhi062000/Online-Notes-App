...
} elseif (!(strlen($_POST["updatepassword"]) > 6
    and preg_match('/[A-Z]/', $_POST["updatepassword"])
    and preg_match('/[0-9]/', $_POST["updatepassword"]))) {
    $errors .= $invalidPassword;
}
else {
    $password = filter_var($_POST["updatepassword"], FILTER_SANITIZE_STRING);
    if (empty($_POST["updatepassword2"])) {
        $errors .= $missingPassword2;
    } else {
        $password2 = filter_var($_POST["updatepassword2"], FILTER_SANITIZE_STRING);
